...
// изменение скорости
void change_speed(char key, int *time_to_sleep);
...
// открываем файл 'in.txt' с инпутом
    ...
    if (error == 0) {
        ...
        while (key_print != 'q') {
            ...
            change_speed(key_print, &time_to_sleep);
        }
    }
...
void change_speed(char key, int *time_to_sleep) {
    int step = 50;
    if (key == 'w' && *time_to_sleep > step)
        *time_to_sleep = *time_to_sleep - step;
    if (key == 's' && *time_to_sleep < step * 100)
        *time_to_sleep = *time_to_sleep + step;
}
